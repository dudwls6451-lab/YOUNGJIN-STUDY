...
// v11.26 제한 자료실: data/resources.json을 읽어 관리자 업로드 패치로 자료를 확장합니다.
let RESOURCE_LIBRARY_ITEMS = [
...
async function loadResourceLibraryManifest() {
  try {
    const response = await fetch("./data/resources.json?v=11.26.0", {cache:"no-store", credentials:"same-origin"});
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    const payload = await response.json();
    const rows = Array.isArray(payload) ? payload : payload?.resources;
    if (!Array.isArray(rows) || rows.length === 0) throw new Error("resources 배열이 비어 있습니다.");
    const normalized = rows
      .filter(item => item && item.id && item.title && Array.isArray(item.pages) && item.pages.length)
      .map(item => ({
        id:String(item.id),
        title:String(item.title),
        tag:String(item.tag || "자료"),
        pages:item.pages.map(page => String(page)).filter(Boolean),
      }))
      .filter(item => item.pages.length);
    if (!normalized.length) throw new Error("유효한 자료가 없습니다.");
    RESOURCE_LIBRARY_ITEMS = normalized;
  } catch (err) {
    console.warn("자료실 목록 파일을 읽지 못해 기본 목록을 사용합니다.", err);
  }
}
...
async function bootstrap() {
  ...
  await loadResourceLibraryManifest();
  await loadBank();
  renderTopProgress();
  showMainModeHub();
}
