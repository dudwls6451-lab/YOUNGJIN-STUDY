...
function hideStudySurfaces() {
  [els.homeHero, els.topProgressCard, els.controlsCard, els.quizCard, els.resultCard, els.statsCard, els.errorsCard, els.adminErrorReportsCard, els.theoryCard, els.resourceLibraryCard, els.resourceViewerCard, els.wrongReviewHubCard, els.wrongReviewFilterCard].forEach(el => el?.classList.add("hidden"));
}
...
const SIDEBAR_COLLAPSE_KEY = "pilotbank-ui-sidebar-collapsed-v1145";

function setSidebarCollapsed(collapsed) {
  const isWide = window.matchMedia("(min-width: 1024px)").matches;
  const next = !!collapsed && isWide;
...
window.addEventListener("resize", () => {
  if (!window.matchMedia("(min-width: 1024px)").matches) document.body.classList.remove("sidebar-collapsed");
  else restoreSidebarCollapsed();
});
