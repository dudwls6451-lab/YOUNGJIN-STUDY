(() => {
  ...
  function errText(error){
    ...
    if(/study_group_dashboard_v2|study_group_goals|study_group_board_|function .* does not exist|schema cache|PGRST202/i.test(msg)) return "스터디그룹 확장 DB가 아직 준비되지 않았습니다. Supabase SQL Editor에서 V11.59.1_STUDY_GROUP_MULTI_GOALS_BOARD.sql을 먼저 실행해 주세요.";
  ...
  async function saveGoal(e){
    e.preventDefault();const g=activeGroup();if(!g?.is_leader)return;const type=$("#studyGoalType").value,title=$("#studyGoalCustomTitle").value.trim(),minutes=type==="study_time"?Number($("#studyGoalMinutes").value||0):0,questions=type==="questions"?Number($("#studyGoalQuestions").value||0):0,subject=type==="theory_stage"?$("#studyGoalSubject").value:null,stage=type==="theory_stage"?Number($("#studyGoalStage").value||0):null,note=$("#studyGoalNote").value||"";
    ...
    try{await rpc("study_group_save_goal",{p_goal_id:state.editingGoalId,p_group_id:g.group_id,p_goal_date:state.dashboardDate,p_goal_type:type,p_title:title,p_target_study_minutes:minutes,p_target_subject:subject,p_target_stage_order:stage,p_target_questions:questions,p_note:note,p_sort_order:state.editingGoalId?(state.goals.findIndex(x=>x.goal_id===state.editingGoalId)+1)*10:(state.goals.length+1)*10});closeGoalEditor();await refreshActive(false);}catch(e2){alert(`목표 저장에 실패했습니다.\n${errText(e2)}`);}
  }
