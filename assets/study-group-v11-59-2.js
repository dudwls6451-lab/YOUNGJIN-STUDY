(() => {
  ...
  function errText(error){
    ...
    if(/study_group_dashboard_v2|study_group_goals|study_group_board_|function .* does not exist|schema cache|PGRST202/i.test(msg)) return "스터디그룹 확장 DB가 아직 준비되지 않았습니다. Supabase SQL Editor에서 V11.59.2_STUDY_GROUP_GOAL_SAVE_HOTFIX.sql까지 실행해 주세요.";
          ...
          <div class="study-group-main">
            ...
            <div id="studyGroupDetail" class="hidden">
              ...
              <section id="studyGroupGoalCard" class="study-group-goal-card">
                ...
                <form id="studyGroupGoalEditor" class="study-group-goal-form study-group-goal-editor hidden">
                  ...
                  <div class="study-group-form-grid">
                    <label><span>목표 종류</span><select id="studyGoalType"><option value="study_time">공부시간</option><option value="theory_stage">이론 학습진도</option><option value="questions">문제 풀이</option></select></label>
                    <label><span>목표 이름</span><input id="studyGoalCustomTitle" type="text" maxlength="120" placeholder="예: 오전 집중공부"></label>
                    <label id="studyGoalMinutesWrap"><span>공부시간</span><div class="study-group-input-with-unit"><input id="studyGoalMinutes" type="number" min="1" max="1440" step="10" value="120"><span>분</span></div></label>
                    <label id="studyGoalQuestionsWrap" class="hidden"><span>문제 풀이</span><div class="study-group-input-with-unit"><input id="studyGoalQuestions" type="number" min="1" max="10000" step="5" value="30"><span>문항</span></div></label>
  ...
  function goalTypeLabel(t){return t==="study_time"?"공부시간":t==="theory_stage"?"이론 진도":t==="questions"?"문제 풀이":"목표";}
  ...
  function updateGoalEditorFields(){
    const type=$("#studyGoalType")?.value;$("#studyGoalMinutesWrap")?.classList.toggle("hidden",type!=="study_time");$("#studyGoalQuestionsWrap")?.classList.toggle("hidden",type!=="questions");$("#studyGoalSubjectWrap")?.classList.toggle("hidden",type!=="theory_stage");$("#studyGoalStageWrap")?.classList.toggle("hidden",type!=="theory_stage");updateStageHint();
  }
  ...
  async function saveGoal(e){
    ...
    const type=$("#studyGoalType").value,title=$("#studyGoalCustomTitle").value.trim(),minutes=type==="study_time"?Number($("#studyGoalMinutes").value||0):0,questions=type==="questions"?Number($("#studyGoalQuestions").value||0):0,subject=type==="theory_stage"?$("#studyGoalSubject").value:null,stage=type==="theory_stage"?Number($("#studyGoalStage").value||0):null,note=$("#studyGoalNote").value||"";
    if(type==="study_time"&&minutes<1){alert("공부시간 목표를 1분 이상 입력해 주세요.");return;}if(type==="questions"&&questions<1){alert("문제 풀이 목표를 1문항 이상 입력해 주세요.");return;}if(type==="theory_stage"&&(!subject||stage<1)){alert("이론 교재와 목표 단계를 입력해 주세요.");return;}
    const sortOrder=state.editingGoalId?(state.goals.findIndex(x=>x.goal_id===state.editingGoalId)+1)*10:(state.goals.length+1)*10;
    const payload={goal_type:type,title,target_study_minutes:minutes,target_subject:subject,target_stage_order:stage,target_questions:questions,note,sort_order:sortOrder};
    if(saveBtn){saveBtn.disabled=true;saveBtn.textContent="저장 중…";}
    try{
      let goalId;
      if(state.editingGoalId){
        goalId=await rpc("study_group_update_goal_v2",{p_goal_id:state.editingGoalId,p_group_id:g.group_id,p_goal:payload});
      }else{
        goalId=await rpc("study_group_add_goal_v2",{p_group_id:g.group_id,p_goal_date:state.dashboardDate,p_goal:payload});
      }
      const rows=await rpc("study_group_goals",{p_group_id:g.group_id,p_goal_date:state.dashboardDate})||[];
      const expectedId=String(goalId||state.editingGoalId||"");
      if(expectedId&&!rows.some(x=>String(x.goal_id)===expectedId))throw new Error("저장 요청은 처리됐지만 새 목표를 다시 불러오지 못했습니다. 잠시 후 현황 갱신을 눌러 주세요.");
      ...
      console.error("[StudyGroup] goal save",e2);
      const msg=String(e2?.message||e2||"");
      if(/study_group_(add|update)_goal_v2|PGRST202|schema cache|function .* does not exist/i.test(msg)){
        alert("목표 저장용 DB 핫픽스가 아직 적용되지 않았습니다. V11.59.2_STUDY_GROUP_GOAL_SAVE_HOTFIX.sql을 Supabase SQL Editor에서 한 번 실행해 주세요.");
      }else alert(`목표 저장에 실패했습니다.\n${errText(e2)}`);
  ...
  async function init(){injectUI();try{await window.PilotBankAuth?.requireLogin?.();}catch{}const nav=$("#studyGroupNavBtn");nav?.classList.toggle("hidden",!hasAccess());if(!hasAccess()){$("#studyGroupInvitesSection")?.classList.add("hidden");return;}wireEvents();await Promise.all([loadInvites(),loadGroups(true)]);}
