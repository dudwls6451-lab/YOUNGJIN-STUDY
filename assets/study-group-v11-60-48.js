/* PilotBank v11.60.48 — Study Group two-stage mode router */
(() => {
  "use strict";
  ...
  const state={
    ...
    groupQuestions:[], questionCategories:[], questionCategoryFilter:"", questionMode:"solve", activeQuestionId:null, questionComments:new Map(), questionLoading:false,
    groupViewStage:"entry", workspaceMode:null
  };
  ...
  const QUESTION_LAB_HTML=`              <section class="study-group-question-section">
                ...
                <div class="study-group-question-filterbar">
                  <label><span>카테고리 보기</span><select id="studyGroupQuestionCategoryFilter"><option value="">전체 카테고리</option><option value="__uncategorized__">미분류</option></select></label>
                </div>
                ...
                <div id="studyGroupQuestionCreatePanel" class="study-group-question-panel hidden">
                  <form id="studyGroupQuestionForm" class="study-group-question-form" novalidate>
                    ...
                    <div class="study-group-question-meta-grid">
                      ...
                      <label><span>카테고리</span><select id="studyGroupQuestionCategory"><option value="">미분류</option></select></label>
                      <label><span>과목/태그</span><input id="studyGroupQuestionTag" maxlength="80" placeholder="예: 공중항법, RVSM"></label>
                    </div>
                    <div class="study-group-category-actions">
                      <button id="studyGroupNewCategoryBtn" class="button secondary mini" type="button">＋ 새 카테고리 만들기</button>
                      <div id="studyGroupCategoryCreator" class="study-group-category-creator hidden">
                        <input id="studyGroupNewCategoryName" maxlength="60" placeholder="예: 항법 계산문제">
                        <button id="studyGroupCategorySaveBtn" class="button mini" type="button">추가</button>
                        <button id="studyGroupCategoryCancelBtn" class="button secondary mini" type="button">취소</button>
                      </div>
                    </div>
  ...
  function ensureQuestionLabUI(){
    ...
    if(existingLab&&(!$("#studyGroupQuestionCreateTab",existingLab)||!$("#studyGroupQuestionCategory",existingLab)))existingLab.remove();
    if(!$("#studyGroupQuestionCreateTab",detail)||!$("#studyGroupQuestionCategory",detail)){
  ...
  function errText(error){
    ...
    if(/study_group_dashboard_v2|study_group_goals|study_group_board_|study_group_question_|study_group_questions|function .* does not exist|schema cache|PGRST202/i.test(msg)) return "스터디그룹 문제은행 DB가 아직 준비되지 않았습니다. Supabase SQL Editor에서 V11.60.45_STUDY_GROUP_QUESTION_LAB.sql과 V11.60.47_STUDY_GROUP_QUESTION_CATEGORIES.sql을 실행해 주세요.";
  ...
  function ensureStudyGroupModeRouterUI(){
    const card=$("#studyGroupCard");
    if(!card)return false;
    const layout=$(".study-group-layout",card);
    if(layout&&!$("#studyGroupEntryHub",card)){
      const hub=document.createElement("section");
      hub.id="studyGroupEntryHub";
      hub.className="study-group-mode-hub";
      hub.innerHTML=`
        <div class="study-group-mode-hub-head">
          <span class="eyebrow">STUDY GROUP MODE</span>
          <h3>스터디 그룹에서 무엇을 할까요?</h3>
          <p class="muted">새 그룹을 만들거나, 가입한 스터디 그룹으로 들어가 학습 모드를 선택하세요.</p>
        </div>
        <div class="study-group-entry-mode-grid">
          <button id="studyGroupEntryCreateMode" class="study-group-mode-card" type="button">
            <span class="study-group-mode-icon">＋</span>
            <strong>스터디 그룹 만들기</strong>
            <span>그룹 이름을 정하고 함께 공부할 회원을 초대합니다.</span>
          </button>
          <button id="studyGroupEntryMyMode" class="study-group-mode-card" type="button">
            <span class="study-group-mode-icon">◫</span>
            <strong>내 스터디 그룹</strong>
            <span>가입한 그룹을 고른 뒤 목표·문제 만들기·문제 풀이 모드로 이동합니다.</span>
          </button>
        </div>`;
      layout.insertAdjacentElement("beforebegin",hub);
    }
    if(layout&&!$("#studyGroupMyGroupsToolbar",card)){
      const toolbar=document.createElement("div");
      toolbar.id="studyGroupMyGroupsToolbar";
      toolbar.className="study-group-my-groups-toolbar hidden";
      toolbar.innerHTML=`<button id="studyGroupBackToEntryBtn" class="button secondary" type="button">← 스터디그룹 메뉴</button><div><strong>내 스터디 그룹</strong><span class="muted">그룹을 선택한 뒤 학습 모드를 고르세요.</span></div>`;
      layout.insertAdjacentElement("beforebegin",toolbar);
    }
    const detail=$("#studyGroupDetail",card);
    if(detail&&!$("#studyGroupWorkspaceModeHub",detail)){
      const detailHead=$(".study-group-detail-head",detail);
      const hub=document.createElement("section");
      hub.id="studyGroupWorkspaceModeHub";
      hub.className="study-group-workspace-mode-hub hidden";
      hub.innerHTML=`
        <div class="study-group-workspace-mode-head">
          <div><span class="eyebrow">CHOOSE MODE</span><h3>학습 모드를 선택하세요.</h3><p class="muted">선택한 스터디 그룹 안에서 할 일을 고릅니다.</p></div>
        </div>
        <div class="study-group-workspace-mode-grid">
          <button class="study-group-workspace-mode-card" type="button" data-study-workspace-mode="goals">
            <span class="study-group-mode-icon">✓</span><strong>오늘의 스터디 목표</strong><span>오늘 목표와 그룹원 달성 현황을 확인하고 관리합니다.</span>
          </button>
          <button class="study-group-workspace-mode-card" type="button" data-study-workspace-mode="create">
            <span class="study-group-mode-icon">✎</span><strong>문제 만들기</strong><span>카테고리를 정해 객관식 문제를 직접 출제합니다.</span>
          </button>
          <button class="study-group-workspace-mode-card" type="button" data-study-workspace-mode="solve">
            <span class="study-group-mode-icon">▶</span><strong>스터디원의 문제 풀기</strong><span>그룹원이 만든 문제를 카테고리별로 풀고 댓글로 토론합니다.</span>
          </button>
        </div>`;
      if(detailHead)detailHead.insertAdjacentElement("afterend",hub);else detail.prepend(hub);
    }
    const modeHub=$("#studyGroupWorkspaceModeHub",detail);
    if(modeHub&&!$("#studyGroupWorkspaceBackBtn",detail)){
      const back=document.createElement("button");
      back.id="studyGroupWorkspaceBackBtn";
      back.className="button secondary study-group-workspace-back hidden";
      back.type="button";
      back.textContent="← 모드 선택";
      modeHub.insertAdjacentElement("afterend",back);
    }
    const goal=$("#studyGroupGoalCard",detail);if(goal)goal.dataset.studyWorkspaceSection="goals";
    const members=$(".study-group-members-section",detail);if(members)members.dataset.studyWorkspaceSection="goals";
    const questions=$(".study-group-question-section",detail);if(questions)questions.dataset.studyWorkspaceSection="questions";
    const board=$(".study-group-board-section",detail);if(board)board.dataset.studyWorkspaceSection="board";
    const datebar=$(".study-group-datebar",detail);if(datebar)datebar.dataset.studyWorkspaceSection="datebar";
    return !!$("#studyGroupEntryHub",card)&&!!$("#studyGroupWorkspaceModeHub",card);
  }

  function showStudyGroupEntry(){
    state.groupViewStage="entry";state.workspaceMode=null;
    $("#studyGroupEntryHub")?.classList.remove("hidden");
    $("#studyGroupMyGroupsToolbar")?.classList.add("hidden");
    $(".study-group-layout")?.classList.add("hidden");
    const create=$("#studyGroupCreateBtn");if(create)create.classList.add("hidden");
    const refresh=$("#studyGroupRefreshBtn");if(refresh)refresh.classList.add("hidden");
  }

  function showMyStudyGroups(){
    state.groupViewStage="groups";state.workspaceMode=null;
    $("#studyGroupEntryHub")?.classList.add("hidden");
    $("#studyGroupMyGroupsToolbar")?.classList.remove("hidden");
    $(".study-group-layout")?.classList.remove("hidden");
    const create=$("#studyGroupCreateBtn");if(create)create.classList.add("hidden");
    const refresh=$("#studyGroupRefreshBtn");if(refresh)refresh.classList.remove("hidden");
    if(state.activeGroupId)showGroupModePicker();else renderNoGroup();
  }

  function setWorkspaceSectionVisibility(mode){
    const detail=$("#studyGroupDetail");if(!detail)return;
    const goal=$("#studyGroupGoalCard",detail),members=$(".study-group-members-section",detail),questions=$(".study-group-question-section",detail),board=$(".study-group-board-section",detail),datebar=$(".study-group-datebar",detail);
    const isGoals=mode==="goals",isCreate=mode==="create",isSolve=mode==="solve";
    goal?.classList.toggle("hidden",!isGoals);
    members?.classList.toggle("hidden",!isGoals);
    datebar?.classList.toggle("hidden",!isGoals);
    questions?.classList.toggle("hidden",!(isCreate||isSolve));
    board?.classList.toggle("hidden",!(isGoals||isCreate||isSolve));
    if(isCreate)setQuestionMode("create");
    if(isSolve)setQuestionMode("solve");
  }

  function showGroupModePicker(){
    if(!state.activeGroupId)return;
    state.groupViewStage="group-mode";state.workspaceMode=null;
    $("#studyGroupDetail")?.classList.remove("hidden");
    $("#studyGroupEmpty")?.classList.add("hidden");
    $("#studyGroupWorkspaceModeHub")?.classList.remove("hidden");
    $("#studyGroupWorkspaceBackBtn")?.classList.add("hidden");
    setWorkspaceSectionVisibility(null);
    window.scrollTo({top:Math.max(0,($("#studyGroupCard")?.offsetTop||0)-70),behavior:"smooth"});
  }

  function enterStudyWorkspace(mode){
    if(!state.activeGroupId)return;
    state.groupViewStage="workspace";state.workspaceMode=mode;
    $("#studyGroupWorkspaceModeHub")?.classList.add("hidden");
    $("#studyGroupWorkspaceBackBtn")?.classList.remove("hidden");
    setWorkspaceSectionVisibility(mode);
    const target=mode==="goals"?$("#studyGroupGoalCard"):$(".study-group-question-section");
  ...
  function openView(){if(!hasAccess())return;hideOtherViews();ensureStudyGroupModeRouterUI();$("#studyGroupCard")?.classList.remove("hidden");showStudyGroupEntry();window.scrollTo({top:0,behavior:"smooth"});loadGroups(true);startRefreshTimer();}
  function closeView(){$("#studyGroupCard")?.classList.add("hidden");showStudyGroupEntry();stopRefreshTimer();}
  ...
  function renderNoGroup(){$("#studyGroupEmpty")?.classList.remove("hidden");$("#studyGroupDetail")?.classList.add("hidden");$("#studyGroupWorkspaceModeHub")?.classList.add("hidden");}
  ...
  async function refreshActive(showLoading=true){
    ...
    try{
      ...
      await loadQuestionCategories(false);
      await Promise.all([loadBoard(false), loadGroupQuestions(false)]);
      if(state.groupViewStage==="group-mode")showGroupModePicker();else if(state.groupViewStage==="workspace"&&state.workspaceMode)enterStudyWorkspace(state.workspaceMode);
  ...
  function resetQuestionForm(){["#studyGroupQuestionText","#studyGroupChoiceA","#studyGroupChoiceB","#studyGroupChoiceC","#studyGroupChoiceD","#studyGroupQuestionTag","#studyGroupQuestionExplanation"].forEach(s=>{const el=$(s);if(el)el.value="";});const c=$("#studyGroupCorrectChoice");if(c)c.value="A";const cat=$("#studyGroupQuestionCategory");if(cat)cat.value="";toggleCategoryCreator(false);}
  function filteredGroupQuestions(){const f=state.questionCategoryFilter;return state.groupQuestions.filter(q=>!f||(f==="__uncategorized__"?!q.category_id:String(q.category_id||"")===String(f)));}
  function renderQuestionCategoryControls(){const make=$("#studyGroupQuestionCategory"),filter=$("#studyGroupQuestionCategoryFilter");const opts=state.questionCategories.map(c=>`<option value="${esc(c.category_id)}">${esc(c.name)} (${Number(c.question_count||0)})</option>`).join("");if(make){const keep=make.value;make.innerHTML=`<option value="">미분류</option>${opts}`;if([...make.options].some(o=>o.value===keep))make.value=keep;}if(filter){const keep=state.questionCategoryFilter||filter.value;filter.innerHTML=`<option value="">전체 카테고리</option><option value="__uncategorized__">미분류</option>${opts}`;if([...filter.options].some(o=>o.value===keep)){filter.value=keep;state.questionCategoryFilter=keep;}else{filter.value="";state.questionCategoryFilter="";}}}
  function toggleCategoryCreator(show){const wrap=$("#studyGroupCategoryCreator");if(!wrap)return;wrap.classList.toggle("hidden",!show);if(show)setTimeout(()=>$("#studyGroupNewCategoryName")?.focus(),0);else{const input=$("#studyGroupNewCategoryName");if(input)input.value="";}}
  async function loadQuestionCategories(showLoading=false){const g=activeGroup();if(!g)return;try{state.questionCategories=await rpc("study_group_question_categories",{p_group_id:g.group_id})||[];renderQuestionCategoryControls();}catch(e){if(showLoading)alert(errText(e));else console.warn("[StudyGroup] categories",e);}}
  async function createQuestionCategory(){const g=activeGroup(),input=$("#studyGroupNewCategoryName"),name=input?.value.trim();if(!g||!name)return;if(name.length>60){alert("카테고리 이름은 60자 이내로 입력해 주세요.");return;}const btn=$("#studyGroupCategorySaveBtn");if(btn)btn.disabled=true;try{const id=await rpc("study_group_question_category_create",{p_group_id:g.group_id,p_name:name});await loadQuestionCategories(false);const select=$("#studyGroupQuestionCategory");if(select&&id)select.value=String(id);toggleCategoryCreator(false);}catch(e){alert(`카테고리를 만들지 못했습니다.\n${errText(e)}`);}finally{if(btn)btn.disabled=false;}}
  ...
  async function loadGroupQuestions(showLoading=true){const g=activeGroup();if(!g)return;state.questionLoading=true;if(showLoading){if($("#studyGroupQuestionPlayer"))$("#studyGroupQuestionPlayer").innerHTML='<div class="notice">그룹 문제를 불러오는 중입니다.</div>';if($("#studyGroupQuestionList"))$("#studyGroupQuestionList").innerHTML='<div class="notice">그룹 문제를 불러오는 중입니다.</div>';}try{state.groupQuestions=await rpc("study_group_questions",{p_group_id:g.group_id,p_limit:200,p_offset:0})||[];const filtered=filteredGroupQuestions();if(!filtered.some(q=>String(q.question_id)===String(state.activeQuestionId)))state.activeQuestionId=filtered[0]?.question_id||null;renderQuestionPlayer();renderQuestionList();}catch(e){const msg=esc(errText(e));if($("#studyGroupQuestionPlayer"))$("#studyGroupQuestionPlayer").innerHTML=`<div class="notice">${msg}</div>`;if($("#studyGroupQuestionList"))$("#studyGroupQuestionList").innerHTML=`<div class="notice">${msg}</div>`;}finally{state.questionLoading=false;}}
  function currentGroupQuestion(){const rows=filteredGroupQuestions();return rows.find(q=>String(q.question_id)===String(state.activeQuestionId))||rows[0]||null;}
  async function createGroupQuestion(e){e.preventDefault();const g=activeGroup();if(!g)return;const question=$("#studyGroupQuestionText")?.value.trim(),a=$("#studyGroupChoiceA")?.value.trim(),b=$("#studyGroupChoiceB")?.value.trim(),c=$("#studyGroupChoiceC")?.value.trim(),d=$("#studyGroupChoiceD")?.value.trim(),correct=$("#studyGroupCorrectChoice")?.value,categoryId=$("#studyGroupQuestionCategory")?.value||null,tag=$("#studyGroupQuestionTag")?.value.trim(),explanation=$("#studyGroupQuestionExplanation")?.value.trim();if(!question||!a||!b||!c||!d){alert("문제와 A~D 선택지를 모두 입력해 주세요.");return;}const btn=e.currentTarget.querySelector('button[type="submit"]');if(btn){btn.disabled=true;btn.textContent="등록 중…";}try{const id=await rpc("study_group_question_create_v2",{p_group_id:g.group_id,p_question:question,p_choice_a:a,p_choice_b:b,p_choice_c:c,p_choice_d:d,p_correct_choice:correct,p_category_id:categoryId,p_explanation:explanation||"",p_tag:tag||""});resetQuestionForm();state.questionCategoryFilter=categoryId?String(categoryId):"__uncategorized__";await Promise.all([loadQuestionCategories(false),loadGroupQuestions(false)]);renderQuestionCategoryControls();state.activeQuestionId=id||filteredGroupQuestions()[0]?.question_id||null;setQuestionMode("solve");await refreshDashboardOnly();}catch(e2){alert(`문제 등록에 실패했습니다.\n${errText(e2)}`);}finally{if(btn){btn.disabled=false;btn.textContent="문제 등록";}}}
  ...
  function renderQuestionPlayer(freshResult=null){const box=$("#studyGroupQuestionPlayer");if(!box)return;const rows=filteredGroupQuestions();if(!rows.length){box.innerHTML=`<div class="study-group-board-empty">${state.groupQuestions.length?"선택한 카테고리에 문제가 없습니다.":"아직 그룹 문제가 없습니다. “문제 만들기”에서 첫 문제를 등록해 보세요."}</div>`;return;}const q=currentGroupQuestion();if(!q)return;const idx=Math.max(0,rows.findIndex(x=>String(x.question_id)===String(q.question_id)));const attempted=!!q.my_attempted||!!freshResult;const correct=freshResult?.correct_choice||q.my_correct_choice||null;const selected=freshResult?.selected_choice||q.my_selected_choice||null;const isCorrect=freshResult?.is_correct??q.my_is_correct;const explanation=freshResult?.explanation??q.my_explanation??"";const choices=[["A",q.choice_a],["B",q.choice_b],["C",q.choice_c],["D",q.choice_d]];box.innerHTML=`<article class="study-group-question-player-card" data-group-question-player data-question-id="${esc(q.question_id)}"><div class="study-group-question-player-head"><div><span class="study-group-goal-kind">${esc(q.category_name||"미분류")}</span><small>${q.tag?`${esc(q.tag)} · `:""}${esc(q.author_username||"회원")} · ${esc(fmtDateTime(q.created_at))}</small></div><span>${idx+1} / ${rows.length}</span></div><h4>${esc(q.question).replace(/\n/g,"<br>")}</h4><div class="study-group-question-answers">${choices.map(([id,text])=>`<button class="study-group-question-answer ${attempted&&id===correct?"correct":""} ${attempted&&id===selected&&id!==correct?"incorrect":""}" type="button" data-answer-choice="${id}" ${attempted?"disabled":""}><b>${id}</b><span>${esc(text)}</span></button>`).join("")}</div>${attempted?`<div class="study-group-question-result ${isCorrect?"good":"bad"}"><strong>${isCorrect?"✓ 정답입니다.":`✕ 오답입니다. 정답 ${esc(correct||"")}`}</strong>${explanation?`<p>${esc(explanation).replace(/\n/g,"<br>")}</p>`:""}</div>`:""}<div class="study-group-question-player-actions"><button class="button secondary mini" type="button" data-question-prev ${idx<=0?"disabled":""}>이전</button><button class="button secondary mini" type="button" data-question-next ${idx>=rows.length-1?"disabled":""}>다음</button><button class="button secondary mini" type="button" data-question-comments>댓글 ${Number(q.comment_count||0)}개</button></div><div id="studyGroupQuestionComments" class="study-group-question-comments ${state.questionComments.has(q.question_id)?"":"hidden"}">${state.questionComments.has(q.question_id)?renderQuestionCommentsHtml(q.question_id):""}</div></article>`;}
  function renderQuestionList(){const list=$("#studyGroupQuestionList");if(!list)return;const rows=filteredGroupQuestions();if(!rows.length){list.innerHTML=`<div class="study-group-board-empty">${state.groupQuestions.length?"선택한 카테고리에 등록된 문제가 없습니다.":"등록된 문제가 없습니다."}</div>`;return;}list.innerHTML=rows.map((q,i)=>`<article class="study-group-question-list-card" data-question-id="${esc(q.question_id)}"><div><span class="study-group-goal-kind">${esc(q.category_name||"미분류")}</span><strong>${i+1}. ${esc(q.question)}</strong><small>${q.tag?`${esc(q.tag)} · `:""}${esc(q.author_username||"회원")} · 풀이 ${Number(q.attempt_count||0)}회 · 댓글 ${Number(q.comment_count||0)}개</small></div><div class="study-group-question-list-actions"><button class="button secondary mini" type="button" data-open-question>풀기</button>${q.can_delete?'<button class="button danger-outline mini" type="button" data-delete-question>삭제</button>':''}</div></article>`).join("");}
  async function deleteGroupQuestion(id){if(!id||!confirm("이 문제를 삭제할까요? 풀이 기록과 댓글도 함께 삭제됩니다."))return;try{await rpc("study_group_question_delete",{p_question_id:id});state.questionComments.delete(id);if(String(state.activeQuestionId)===String(id))state.activeQuestionId=null;await Promise.all([loadQuestionCategories(false),loadGroupQuestions(false)]);await refreshDashboardOnly();}catch(e){alert(errText(e));}}
  ...
  function wireEvents(){
    $("#studyGroupEntryCreateMode")?.addEventListener("click",()=>openMemberModal("create"));
    $("#studyGroupEntryMyMode")?.addEventListener("click",()=>{showMyStudyGroups();loadGroups(true);});
    $("#studyGroupBackToEntryBtn")?.addEventListener("click",showStudyGroupEntry);
    $("#studyGroupWorkspaceModeHub")?.addEventListener("click",e=>{const b=e.target.closest("[data-study-workspace-mode]");if(b)enterStudyWorkspace(b.dataset.studyWorkspaceMode);});
    $("#studyGroupWorkspaceBackBtn")?.addEventListener("click",showGroupModePicker);
    ...
    $("#studyGroupQuestionSolveQuickBtn")?.addEventListener("click",()=>enterStudyWorkspace("solve"));$("#studyGroupQuestionCreateQuickBtn")?.addEventListener("click",()=>enterStudyWorkspace("create"));
    $(".study-group-question-tabs")?.addEventListener("click",e=>{const b=e.target.closest("[data-question-mode]");if(b)setQuestionMode(b.dataset.questionMode);});$("#studyGroupQuestionCategoryFilter")?.addEventListener("change",e=>{state.questionCategoryFilter=e.target.value||"";const rows=filteredGroupQuestions();state.activeQuestionId=rows[0]?.question_id||null;renderQuestionPlayer();renderQuestionList();});$("#studyGroupQuestionRefreshBtn")?.addEventListener("click",()=>Promise.all([loadQuestionCategories(true),loadGroupQuestions(true)]));$("#studyGroupNewCategoryBtn")?.addEventListener("click",()=>toggleCategoryCreator(true));$("#studyGroupCategoryCancelBtn")?.addEventListener("click",()=>toggleCategoryCreator(false));$("#studyGroupCategorySaveBtn")?.addEventListener("click",createQuestionCategory);$("#studyGroupNewCategoryName")?.addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();createQuestionCategory();}else if(e.key==="Escape")toggleCategoryCreator(false);});$("#studyGroupQuestionForm")?.addEventListener("submit",createGroupQuestion);$("#studyGroupQuestionFormReset")?.addEventListener("click",resetQuestionForm);$("#studyGroupQuestionPlayer")?.addEventListener("click",e=>{const card=e.target.closest("[data-group-question-player]");if(!card)return;const id=card.dataset.questionId;if(e.target.closest("[data-answer-choice]"))answerGroupQuestion(id,e.target.closest("[data-answer-choice]").dataset.answerChoice,e.target.closest("[data-answer-choice]"));else if(e.target.closest("[data-question-prev]")){const rows=filteredGroupQuestions(),i=rows.findIndex(q=>String(q.question_id)===String(id));if(i>0){state.activeQuestionId=rows[i-1].question_id;renderQuestionPlayer();}}else if(e.target.closest("[data-question-next]")){const rows=filteredGroupQuestions(),i=rows.findIndex(q=>String(q.question_id)===String(id));if(i>=0&&i<rows.length-1){state.activeQuestionId=rows[i+1].question_id;renderQuestionPlayer();}}else if(e.target.closest("[data-question-comments]"))toggleQuestionComments(id);else{const c=e.target.closest("[data-delete-question-comment]");if(c)deleteQuestionComment(id,c.closest("[data-question-comment-id]")?.dataset.questionCommentId);}});$("#studyGroupQuestionPlayer")?.addEventListener("submit",e=>{const form=e.target.closest("[data-question-comment-form]");if(!form)return;e.preventDefault();const id=form.closest("[data-group-question-player]")?.dataset.questionId;if(id)createQuestionComment(id,form);});$("#studyGroupQuestionList")?.addEventListener("click",e=>{const card=e.target.closest("[data-question-id]");if(!card)return;const id=card.dataset.questionId;if(e.target.closest("[data-open-question]")){state.activeQuestionId=id;setQuestionMode("solve");}else if(e.target.closest("[data-delete-question]"))deleteGroupQuestion(id);});
