(() => {
  // v11.60.43: 1시간 무동작 자동 로그아웃 + 1분 무동작 후 상단 카운트다운 + 수동 연장.
  ...
  const IDLE_COUNTDOWN_START_MS = 60 * 1000;
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .auth-session-timer {
        ...
        padding: 4px 5px 4px 10px; border: 1px solid #d9e0ec; border-radius: 999px;
        background: rgba(255,255,255,.9); color: #334155; white-space: nowrap;
        box-shadow: 0 2px 8px rgba(15,23,42,.06); order: -2;
      }
      .auth-session-timer-label { font-size: .74rem; font-weight: 800; color: #64748b; }
      .auth-session-timer-value {
        min-width: 44px; text-align: center; font-variant-numeric: tabular-nums;
        font-size: .9rem; letter-spacing: .02em; color: #0f172a;
      }
      .auth-session-timer.is-counting { border-color: #f0c36a; background: #fffaf0; }
      .auth-session-timer.is-counting .auth-session-timer-label { color: #8a5a00; }
      .auth-session-timer.is-warning { border-color: #f59e0b; background: #fff7e6; }
      .auth-session-timer.is-warning .auth-session-timer-value { color: #b45309; }
      .auth-session-timer.is-extended { border-color: #86cfa3; background: #f0fff6; }
      .auth-session-extend-button {
        border: 0; border-radius: 999px; padding: 6px 9px; background: #2463eb; color: #fff;
        cursor: pointer; font: inherit; font-size: .76rem; font-weight: 900; line-height: 1;
      }
      .auth-session-extend-button:hover { filter: brightness(.97); }
      .auth-session-extend-button:focus-visible { outline: 2px solid rgba(36,99,235,.35); outline-offset: 2px; }
      @media (max-width: 760px) {
        .auth-session-timer { order: -3; }
        .auth-session-timer-label { display: none; }
        .auth-session-timer-value { min-width: 41px; }
      }
  ...
  function idleRemainingMs(now = Date.now()) {
    if (!idleLastActivityAt) return IDLE_TIMEOUT_MS;
    return Math.max(0, IDLE_TIMEOUT_MS - (now - idleLastActivityAt));
  }

  function renderIdleTimer() {
    const box = document.querySelector("#authSessionTimer");
    const value = document.querySelector("#authSessionTimerValue");
    const label = document.querySelector("#authSessionTimerLabel");
    if (!box || !value || !label || !currentIdentity) return;

    const now = Date.now();
    const idleFor = Math.max(0, now - (idleLastActivityAt || now));
    const remaining = idleRemainingMs(now);
    const counting = idleFor >= IDLE_COUNTDOWN_START_MS;
    const flashExtended = now < idleExtendFlashUntil;

    box.classList.toggle("is-counting", counting && !flashExtended);
    box.classList.toggle("is-warning", counting && remaining <= 5 * 60 * 1000 && !flashExtended);
    box.classList.toggle("is-extended", flashExtended);

    if (flashExtended) {
      label.textContent = "연장됨";
      value.textContent = "60:00";
    } else if (!counting) {
      label.textContent = "자동 로그아웃";
      // 화면 동작이 멈춘 뒤 첫 1분 동안은 카운트다운을 정지해 둡니다.
      value.textContent = "60:00";
    } else {
      label.textContent = "자동 로그아웃";
      value.textContent = formatIdleRemaining(remaining);
    }

    box.title = counting
      ? `1시간 무동작 시 자동 로그아웃 · 남은 시간 ${formatIdleRemaining(remaining)}`
      : "화면에서 1분 이상 동작이 없으면 남은 시간이 카운트다운됩니다.";

    if (remaining <= 0 && !logoutInProgress) performLogout("idle");
  }

  function armIdleLogout() {
    if (!currentIdentity) return;
    if (!idleLastActivityAt) idleLastActivityAt = Date.now();

    if (idleTimer !== null) clearTimeout(idleTimer);
    const delay = Math.max(0, idleRemainingMs());
    idleTimer = window.setTimeout(() => performLogout("idle"), delay);

    if (idleTicker === null) {
      idleTicker = window.setInterval(renderIdleTimer, IDLE_TICK_MS);
    }
    renderIdleTimer();
  }
  ...
  function resetIdleTimer(source = "activity") {
    ...
    if (source === "extend") idleExtendFlashUntil = idleLastActivityAt + 1300;
    armIdleLogout();
  }

  function installIdleLogout() {
    if (!idleListenersInstalled) {
      const markActivity = () => resetIdleTimer("activity");
      ...
      document.addEventListener("visibilitychange", () => {
        if (document.visibilityState === "visible") {
          if (idleRemainingMs() <= 0) performLogout("idle");
          else renderIdleTimer();
        }
      });
  ...
  function addUserControls(identity) {
    ...
    sessionTimer.id = "authSessionTimer";
    sessionTimer.className = "auth-session-timer";
    sessionTimer.setAttribute("aria-label", "자동 로그아웃 남은 시간");
    sessionTimer.innerHTML = `
      <span class="auth-session-timer-label" id="authSessionTimerLabel">자동 로그아웃</span>
      <strong class="auth-session-timer-value" id="authSessionTimerValue">60:00</strong>
      <button class="auth-session-extend-button" id="authSessionExtendBtn" type="button" title="자동 로그아웃 시간을 1시간으로 연장">연장</button>
    `;
    sessionTimer.querySelector("#authSessionExtendBtn")?.addEventListener("click", event => {
      event.preventDefault();
      event.stopPropagation();
      resetIdleTimer("extend");
    });
    ...
    const draw = () => {
      ...
      overlay.innerHTML = `
        <form class="auth-panel" id="emailAuthForm" autocomplete="on">
          ...
          ${idleNotice ? '<div class="auth-notice">1시간 동안 활동이 없어 자동 로그아웃되었습니다.</div>' : ''}
  ...
  window.PilotBankAuth = {
    ...
    extendSession: () => resetIdleTimer("extend"),
    getSessionRemainingMs: () => idleRemainingMs(),
  };
})();
