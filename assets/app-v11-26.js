...
// v11.47 Aviwiki · 이론 백과사전
const AVIWIKI_DATA_PATH = "./data/aviwiki-aerodynamics-v1.json";
...
function aviwikiSubjectDefinitions() {
  const aeroMeta = aviwikiData?.metadata || {};
  const aeroReady = !!aviwikiData?.sections?.length;
  return [
    {key:"Aerodynamics", title:"Aerodynamics", ready:aeroReady, meta:aeroReady ? `${aeroMeta.chapter_count || 0}챕터 · ${aeroMeta.section_count || 0}섹션` : "자료 불러오는 중"},
    {key:"Weather", title:"Weather", ready:false, meta:"자료 준비 중"},
    {key:"Navigation", title:"Navigation", ready:false, meta:"자료 준비 중"},
    {key:"IFR Procedures", title:"IFR Procedures", ready:false, meta:"자료 준비 중"},
    {key:"Systems", title:"Systems", ready:false, meta:"자료 준비 중"},
    {key:"Performance & Weight Balance", title:"Performance & Weight Balance", ready:false, meta:"자료 준비 중"},
    {key:"Emergency & Aeromedical", title:"Emergency & Aeromedical", ready:false, meta:"자료 준비 중"},
    {key:"항공법규", title:"항공법규", ready:false, meta:"자료 준비 중"},
  ];
}
...
async function ensureAviwikiData() {
  ...
  const response = await fetch(`${AVIWIKI_DATA_PATH}?v=11.47.0`, {cache:"no-store"});
  if (!response.ok) throw new Error(`Aviwiki data HTTP ${response.status}`);
  aviwikiData = await response.json();
  if (!Array.isArray(aviwikiData?.sections)) throw new Error("Aviwiki sections missing");
...
function renderAviwikiResume() {
  ...
  if (els.aviwikiHomeMeta) {
    els.aviwikiHomeMeta.textContent = `${aviwikiData.metadata?.chapter_count || 0}챕터 · ${aviwikiData.metadata?.section_count || 0}개 섹션. 궁금한 용어·개념을 검색하거나 Aerodynamics를 처음부터 공부할 수 있어요.`;
  }
}
...
function renderAviwikiToc() {
  ...
  const filteredIds = query ? new Set(aviwikiSearch(query).map(s => s.id)) : null;
  const chapters = aviwikiData.chapters || [];
  let html = "";
  chapters.forEach(chapter => {
    ...
    html += `<section class="aviwiki-toc-group"><div class="aviwiki-toc-group-title">AERODYNAMICS · ${escapeHtml(chapter.title)}</div>`;
...
async function openAviwikiSection(sectionId) {
  ...
  const sectionIndex = aviwikiData.sections.findIndex(item => item.id === section.id);
  if (els.aviwikiReaderProgress) els.aviwikiReaderProgress.textContent = `${sectionIndex + 1} / ${aviwikiData.sections.length}`;
  ...
  if (els.aviwikiNextBtn) {
    els.aviwikiNextBtn.disabled = sectionIndex >= aviwikiData.sections.length - 1;
    els.aviwikiNextBtn.textContent = sectionIndex >= aviwikiData.sections.length - 1 ? "마지막 이론" : "다음 이론 →";
  }
  renderAviwikiToc();
  renderAviwikiResume();
  saveAviwikiState();
...
function navigateAviwiki(delta) {
  ...
  const current = aviwikiData.sections.findIndex(section => section.id === aviwikiCurrentSectionId);
  const next = Math.max(0, Math.min(aviwikiData.sections.length - 1, current + delta));
  if (next === current) return;
  openAviwikiSection(aviwikiData.sections[next].id);
}
...
els.aviwikiSubjectGrid?.addEventListener("click", async event => {
  ...
  await ensureAviwikiData();
  if (item.dataset.aviwikiSubject === "Aerodynamics" && aviwikiData.sections[0]) openAviwikiSection(aviwikiData.sections[0].id);
});
