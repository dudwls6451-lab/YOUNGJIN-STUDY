(() => {
  ...
  // v11.37: 기존 시험용 계정은 김영진_시험용만 유지합니다.
  // 신규 사용자는 이메일+비밀번호로 로그인하고, 회원가입 시에만 이메일 인증코드를 확인합니다.
  const LEGACY_USERS = {
    ...
    const draw = () => {
      ...
      overlay.innerHTML = `
        <form class="auth-panel" id="emailAuthForm" autocomplete="on">
          <h2>항공 문제은행</h2>
          <p>${signup
            ? "이름·이메일·비밀번호를 입력한 뒤 이메일 인증을 완료하면 가입 신청이 접수됩니다."
      ...
      form?.addEventListener("submit", async event => {
        ...
        try {
          ...
          // Confirm Email이 켜져 있어야 가입 시 세션이 즉시 발급되지 않습니다.
          if (data?.session) {
            try { await client().auth.signOut(); } catch {}
            throw new Error("이메일 인증 설정을 확인해 주세요. Supabase의 Confirm Email이 ON이어야 합니다.");
          }

          successBox.textContent = `${email}로 회원가입 인증코드를 보냈습니다.`;
          renderSignupOtpEntry(overlay, resolve, { email, name, draw });
        } catch (err) {
          console.error("[Auth] email auth", err);
          const message = String(err?.message || "");
          if (!signup && /email not confirmed/i.test(message)) {
            errorBox.textContent = "이메일 인증이 아직 완료되지 않았습니다. 회원가입 때 받은 인증코드를 먼저 확인해 주세요.";
  ...
  function renderSignupOtpEntry(overlay, resolve, context) {
    const { email, draw } = context;
    overlay.innerHTML = `
      <form class="auth-panel" id="otpVerifyForm" autocomplete="one-time-code">
        ...
        <button class="auth-submit" id="authVerifyOtp" type="submit">이메일 인증 완료</button>
        <button class="auth-submit secondary" id="authResendOtp" type="button">인증코드 다시 받기</button>
        <button class="auth-submit ghost" id="authOtpBack" type="button">회원가입 화면으로 돌아가기</button>
        <div class="auth-error" id="authOtpError" role="alert"></div>
        <div class="auth-success" id="authOtpSuccess" role="status">이메일 인증이 끝나면 관리자 승인 대기 상태가 됩니다.</div>
      </form>
    `;
    ...
    const resendBtn = overlay.querySelector("#authResendOtp");
    const errorBox = overlay.querySelector("#authOtpError");
    const successBox = overlay.querySelector("#authOtpSuccess");
    ...
    resendBtn?.addEventListener("click", async () => {
      ...
      try {
        ...
        if (error) throw error;
        successBox.textContent = "새 인증코드를 보냈습니다.";
      } catch (err) {
        console.error("[Auth] resend signup OTP", err);
        const message = String(err?.message || "");
        errorBox.textContent = /rate|seconds/i.test(message)
          ? "인증코드 요청이 너무 잦습니다. 잠시 후 다시 시도해 주세요."
          : (message || "인증코드 재전송 중 오류가 발생했습니다.");
      } finally {
        if (resendBtn?.isConnected) resendBtn.disabled = false;
      }
    });
