...
- `각 교재 학습 → 이론 학습`에 **항공교통통신** 과정 추가
- 현재 항공교통통신 문제은행 300문항과 1:1 연계된 **14단계** 이론 과정
- 각 단계 본문을 끝까지 읽으면 기존 문제은행에서 **10문항 무작위 쪽지시험** 출제
- **8문항 이상 정답** 시 다음 단계 해제
- 이론 데이터: `data/theory-atccomm-all.json`
- 교재의 후속 범위(비상절차·항공통신업무·항공정보업무 등)는 대응 문제은행 확장 후 후속 단계로 추가 가능

## v11.28 추가 기능

- 상단 `항공 문제은행` 제목을 누르면 언제든 학습 모드 홈으로 이동합니다.
- 자료실 진입을 상단바 버튼에서 학습 모드 선택 카드로 이동했습니다. 기존 5개 허용 계정에서만 카드가 표시됩니다.
- `오답 복습` 학습 모드를 추가했습니다.
  - 교재(ATP Gleim / 검댕이 항공법규 / 항공기상 / 항공교통통신)만 선택
  - 누적 오답 횟수 `N회 이상`으로 필터
  - 조건에 맞는 문제를 전부 무작위 순서로 반복 출제
  - 결과 화면에서 `이번 회차 오답만 다시` 또는 `같은 조건으로 다시 복습` 가능


## v11.29 자료실 정적 이미지 카드
- 사용자가 제공한 PDF 10종을 180dpi PNG로 직접 렌더링하여 `assets/resources/`에 포함했습니다.
- 자료실 카드에 각 PDF 1페이지의 축소 썸네일을 표시합니다.
- 열람 시 PDF 원본이 아니라 PNG 이미지만 Canvas에 표시합니다.
- 관리자 PDF 업로드/패치 생성 기능은 제거했습니다.


## v11.32 hotfix
- v11.29 안정 UI를 그대로 사용합니다.
- 자료실 이미지 30개만 assets/ 바로 아래로 평탄화했습니다.
- 새 app 파일/CSS/UI 구조를 만들지 않고 기존 app-v11-18.js의 자료실 경로만 수정했습니다.

## v11.33 UI recovery
- 패치 단독 적용 시 누락될 수 있었던 본문 CSS를 `assets/style-v11-33.css`로 패치에 직접 포함했습니다.
- 자료실 이미지는 `assets/` 바로 아래에 유지합니다.


## v11.34 — 항공교통통신 전체판
- 항공교통통신 전체 938문항을 원본 교재 정답표/해설과 대조해 정답 미확정 항목을 복원하고 OCR 오염을 정리했습니다.
- 항공교통통신 문제은행은 출제예상문제 563문항 + 모의고사 15회 375문항입니다.
- 이론학습은 교재 이론 전 범위를 25단계로 확장하고 출제예상문제 563문항을 각 단계 쪽지시험에 1회씩 연결했습니다.
- UI 안정성을 위해 기존 assets/app-v11-18.js와 assets/style-v11-33.css 파일명을 유지하고 데이터/문구만 최소 수정했습니다.
- 새 하위 폴더는 만들지 않았습니다.
