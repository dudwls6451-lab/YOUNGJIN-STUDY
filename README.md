...
v9에서 GitHub Pages 또는 브라우저 캐시 때문에 새 `index.html`과 이전 `app.js`가 혼합될 수 있던 문제를 방지하기 위해,
메인 페이지가 새 자산 파일명을 직접 사용합니다.

- `assets/app-v10.js`
- `assets/style-v10.css`

따라서 기존 `app.js` 캐시와 무관하게 v10 코드가 로드됩니다.

또한 v2~v9에서 저장한 기존 LocalStorage 학습 기록을 자동 보정하여
`examExcluded` 필드가 없는 예전 기록도 `!` 기능과 통계에서 정상 처리합니다.

`!` 활성 시 버튼이 진한 빨강으로 바뀌며, 해당 문제는 시험모드에서 제외됩니다.
